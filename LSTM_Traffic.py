...
class DS_LSTM(Module):
    ...
    def predict(self, X):
        with no_grad():
            # Make sure X is 3D; if it's not, add a batch dimension.
            if X.dim() == 2:
                X = X.unsqueeze(0)  # Add a batch dimension at the beginning
            elif X.dim() == 1:
                X = X.view(1, -1, 1)  # Reshape to [1, sequence_length, features]
            y_pred = self(X)
        return y_pred[:, -1, :]
...
scaler = MinMaxScaler()
data = scaler.fit_transform(datapre)
data = DataFrame(data, index=datapre.index, columns=datapre.columns)
...
tight_layout()
savefig(f"images/{file_tag}_lstms_{measure}_eval.png", dpi=300)
# %%
...
tight_layout()
savefig(f"images/{file_tag}_lstms_{measure}_forecast.png", dpi=300)
